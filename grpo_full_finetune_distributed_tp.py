...
def load_reward_server(reward_fn):
    ...
    if isinstance(reward_fn, str):
        ...
        module_path = reward_fn.get("module", "rewards_gsm8k_default")
...
class FullGRPOFinetuneRecipeDistributed(FTRecipeInterface):
    ...
    def __init__(self, cfg: DictConfig) -> None:
        ...
        # Calculate data parallel dimension
        ...
        self._use_pbar = cfg.get("pbar", False)
            
        RewardServer = load_reward_server(cfg.get('reward_fn', 'gsm8k_default'))
    ...
    def setup(self, cfg: DictConfig) -> None:
        ...
        self._total_steps = cfg.get('num_steps', None)
    ...
    def _setup_model(
        ...
        if self.tensor_parallel_dim > 1:
            ...
            ref_model = training.prepare_mha_for_tp(ref_model, device_mesh["tp"])
        ...
        common_shard_args = dict(
            shard_conditions=fsdp_shard_conditions,
            cpu_offload=fsdp_cpu_offload
        )
        if self.data_parallel_dim > 1:
            common_shard_args['dp_mesh'] = device_mesh['dp']
        model_shard_args = dict(
            model=model,
            reshard_after_forward=False,
            **common_shard_args
        )
        ref_model_shard_args = dict(
            model=ref_model,
            reshard_after_forward=True,
            **common_shard_args
        )

        training.shard_model(**model_shard_args)
        training.shard_model(**ref_model_shard_args)
    ...
    def generate_trajectory(
        ...
        rewards, successes = self.reward_server.batch_shaped_correctness_reward(
    ...
    def train(self) -> None:
        ...
        for curr_epoch in range(self._epochs_run, self.total_epochs):
            ...
            for idx, batch in enumerate(self._dataloader):
                ...
                for ppo_epoch_idx in range(self._ppo_epochs):

                    step_stats = self.grpo_step(trajectory, context_length)
                    # if self._is_rank_zero:
                    #     print(f'\nGRPO step / PPO epoch {ppo_epoch_idx} completed.\n')
                    ...
                    # if self._clip_grad_norm is not None:
                        ...
                        # )
                        # Model should already be wrapped with FSDP. TODO: Validate correctness.
                        # grad_norm = self._model.clip_grad_norm_(max_norm=float(self._clip_grad_norm))
                ...
                if self._is_rank_zero and self._use_pbar:
                    pbar.update(1)
